...
production — `next dev` appends it to `script-src` only, for dev-mode
HMR/React dev eval (bundler-agnostic: gated on `NODE_ENV`, not on
Turbopack vs. webpack); shipped bundles never eval — no
