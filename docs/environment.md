...
| `BLOB_READ_WRITE_TOKEN` | server-only | yes (media) | all | Blob credential for uploads (src). `issueSignedToken` does not reliably pick up platform-injected OIDC (`VERCEL_OIDC_TOKEN`) even with `autoExposeSystemEnvs`/`oidcTokenConfig` enabled on the project — a missing token fails every upload with "No blob credentials found" in local dev, or the opaque client error "Failed to retrieve the presigned URL" in Preview/Production. Set explicitly for every environment; connecting a store to a project does NOT always add this var automatically (it may connect OIDC-only, exposing just `BLOB_STORE_ID`/`BLOB_WEBHOOK_PUBLIC_KEY`). Create a Read-Write token in Vercel Dashboard → Storage → store → `.env.local` tab (prefer a separate dev store for local) | Secret, never commit; verify presence per-environment after connecting a new store |
