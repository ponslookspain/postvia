...
| `domain/social/policies/` | Pure per-platform policies: X error classification + media router + attempt/retry decisions (`x`), Threads auth classification + media router (`threads`). Zero deps (only `domain/media` types). `src/lib/social/{x,threads}.ts` and `src/lib/media.ts` (Threads policy) are re-export shims; OAuth/refresh/Prisma/fetch/upload stay in `lib` |
