...
// PostVIA visual contract: expanded 16rem, collapsed icon 3rem,
// mobile 18rem. (Upstream defaults are 16.25rem / 3.75rem.)
const SIDEBAR_WIDTH_DEFAULT = "16rem"
const SIDEBAR_WIDTH_MIN = "12rem"
const SIDEBAR_WIDTH_MAX = "28rem"
const SIDEBAR_WIDTH_MOBILE = "18rem"
// PostVIA visual contract: collapsed icon rail 3rem (upstream 3.75rem).
...
export type SidebarTriggerProps = Omit<
	// PostVIA adapt: legacy Button API (global button.tsx stays frozen).
...
export function useIsMobile(mobileBreakpoint = 768) {
	...
	React.useEffect(() => {
		...
		// eslint-disable-next-line react-hooks/set-state-in-effect -- sync matchMedia snapshot on mount
		setIsMobile(window.innerWidth < mobileBreakpoint)
...
function SidebarTrigger({
	...
	// PostVIA adapt: inline handler so the event type infers from the
	// legacy Button onClick (BaseUI event), not React.MouseEvent.
	return (
		// PostVIA adapt: legacy Button (ghost/icon-sm preserves visuals).
		<Button
...
function SidebarMenuSkeleton({
	...
	// Random width between 50 to 90% (lazy initializer, same as previous).
