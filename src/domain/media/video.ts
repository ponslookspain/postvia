 ...
 * boundary is the container signature check (`signature.ts`), which has
 ...
 *
 * DOMAIN RULE: no environment access here. The optional duration cap is
 * supplied by the caller (`limitSeconds`); reading `MEDIA_MAX_VIDEO_SECONDS`
 * stays in `src/lib/media-video.ts`.
 */
...
/**
 * Applies the duration cap supplied by the caller.
 * A null duration or null cap always passes. The cap itself is read from
 * the environment by the infrastructure adapter (`src/lib/media-video.ts`),
 * never here.
 */
